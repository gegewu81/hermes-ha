...
19. **Epoch counter exists but lacks comparison logic** — `set_role()` increments epoch on role change, but `takeover()` doesn't compare peer epoch before claiming primary. In a network-split scenario where both nodes think they're primary with same epoch, there's no arbitration. TODO: before takeover, read peer state, if peer epoch >= local epoch → warn or require manual confirmation.
20. **Source .py can be overwritten by push, but .pyc survives** — If Pi writes v2 code but WSL's `push` overwrites it with v1, the compiled `__pycache__/ha_sync.cpython-313.pyc` may still contain v2 bytecode (check mtime and size). Recovery: load .pyc via `importlib.util.spec_from_file_location()` to extract function signatures, docstrings, string constants. `uncompyle6`/`decompyle3` do NOT support Python 3.13 bytecode. Best approach: reconstruct from v1 source + extracted v2 metadata.
